...
import type { DayLog, EventType, HealthEvent } from '@/lib/types';
import {
  addDays,
  capitalize,
  daysBetween,
...
import { EVENT_META, EVENT_ORDER } from '@/lib/events';
    ...
    <>
      ...
      >
        ...
        {Array.from({ length: total }).map((_, i) => {
          ...
          const logged = dayByKey.get(key);
          const sev = logged?.overallSeverity ?? null;
          // Fill: severity ink for symptom days, Floresta green for a logged
          // tranquilo day, white (card) only when there's no record at all.
          const fill = sev ? SEV[sev] : logged ? COLORS.accent : null;
          const filled = fill !== null;
          const dayEvents = eventsByKey.get(key) ?? [];
          const hasEvents = dayEvents.length > 0;
          const dim = !matches(key);
          const dayNum = i + 1;
          // Marker ink is neutral: white over any filled cell, ink otherwise.
          // NEVER a severity ink (those are sacred to symptoms).
          const markerInk = filled ? '#fff' : COLORS.ink;
          // Selection outline stays visible: ink on the green tranquilo fill,
          // accent everywhere else.
          const outlineColor = filled && !sev ? COLORS.ink : COLORS.accent;
          return (
            <button
              key={key}
              ...
              style={{
                ...
                padding: 4,
                border: hasEvents
                  ? `1.5px solid ${filled ? '#fff' : COLORS.ink}`
                  : `1.5px solid ${filled ? 'transparent' : COLORS.line}`,
                background: fill ?? COLORS.card,
                opacity: dim ? 0.25 : 1,
                cursor: 'pointer',
                outline: selected === key ? `2.5px solid ${outlineColor}` : 'none',
                outlineOffset: 1,
                display: 'flex',
      ...
      >
        ...
        >
          <span style={{ width: 11, height: 11, borderRadius: 3, background: COLORS.accent }} />
          tranquilo
        </span>
